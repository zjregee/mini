    ...
    fn poll_next(
        ...
        match self.listener.accept() {
            ...
            Err(ref e) if e.kind() == io::ErrorKind::WouldBlock => {
                ...
                reactor.borrow_mut().modify_readable(self.listener.as_raw_fd(), cx);
                Poll::Pending
            }
        ...
        unsafe {
            ...
            match self.stream.read(b) {
                ...
                Err(ref e) if e.kind() == std::io::ErrorKind::WouldBlock => {
                    let reactor = get_reactor();
                    reactor.borrow_mut().modify_readable(self.stream.as_raw_fd(), cx);
                    Poll::Pending
                }
                Err(e) => {
                    Poll::Ready(Err(e))
...
impl tokio::io::AsyncWrite for TcpStream {
    fn poll_write(
        ...
        match self.stream.write(buf) {
            ...
            Err(ref e) if e.kind() == std::io::ErrorKind::WouldBlock => {
                let reactor = get_reactor();
                reactor.borrow_mut().modify_writable(self.stream.as_raw_fd(), cx);
                Poll::Pending
            }
